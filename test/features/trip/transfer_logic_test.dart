...
void main() {
    ...
    test('间隔 < 20min → 换乘紧张', () {
      final a = evaluateTransfer(
        const TripLeg(
            ...
            arrivalStation: '上海虹桥',
            departAbsMinutes: 390,
            arriveAbsMinutes: 684),
        const TripLeg(
            trainNum: 'G2',
            departureStation: '上海虹桥',
            arrivalStation: '杭州东',
            departAbsMinutes: 703,
            arriveAbsMinutes: 800),
      );
  ...
  group('同城异站（12306 同城组）', () {
    String? cityOf(String s) =>
        (s == '上海虹桥' || s == '上海南' || s == '上海') ? '上海市' : null;
    ...
    test('命中 → 到站前 10min 提示换座', () {
      final a = evaluateTransfer(
        const TripLeg(
            ...
            departureStation: 'A',
            arrivalStation: 'B',
            departAbsMinutes: 300,
            arriveAbsMinutes: 600),
        const TripLeg(
            trainNum: 'G1202',
            departureStation: 'B',
            arrivalStation: 'C',
            departAbsMinutes: 660,
            arriveAbsMinutes: 800),
      );
    ...
    test('默认 18h：超过 → none 断开', () {
      final a = evaluateTransfer(
        ...
        const TripLeg(
            ...
            departureStation: 'B',
            arrivalStation: 'C',
            departAbsMinutes: 600 + 18 * 60 + 1,
            arriveAbsMinutes: 2000),
      );
    ...
    test('时间重叠（gap<0）→ none', () {
      final a = evaluateTransfer(
        ...
        const TripLeg(
            ...
            departureStation: 'B',
            arrivalStation: 'C',
            departAbsMinutes: 500,
            arriveAbsMinutes: 700),
      );
      expect(a.type, TransferType.none);
