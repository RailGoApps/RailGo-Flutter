...
class Sm4Engine {
  Sm4Engine(List<int> key) {
    if (key.length != 16) {
      throw ArgumentError(
          'SM4 key must be 16 bytes (128 bit), got ${key.length}');
    }
...
class Sm4Cipher {
  ...
  Uint8List encrypt(List<int> data) {
    ...
    if (mode == Sm4Mode.ecb) {
      ...
      for (var i = 0; i < padded.length; i += 16) {
        final c =
            _engine.encryptBlock(Uint8List.sublistView(padded, i, i + 16));
  ...
  Uint8List decrypt(List<int> data) {
    if (mode == Sm4Mode.ecb) {
      ...
      for (var i = 0; i < data.length; i += 16) {
        final p = _engine.decryptBlock(
            Uint8List.sublistView(Uint8List.fromList(data), i, i + 16));
  ...
  String encryptStringToBase64(String plain) =>
      base64Encode(encrypt(utf8.encode(plain)));

  String decryptStringFromBase64(String b64) =>
      utf8.decode(decrypt(base64Decode(b64)));
...
/// 便捷入口：以 hex 密钥构造
Sm4Cipher sm4CipherFromHexKey(String hexKey,
    {Sm4Mode mode = Sm4Mode.cbc, Uint8List? iv}) {
